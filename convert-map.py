...
if mapelt.getAttribute('orientation') != 'orthogonal':
    ...
    sys.exit(-1)
if int(mapelt.getAttribute('tileheight')) != 32:
    print 'tile height must be 32, not', mapelt.getAttribute('tileheight')
    sys.exit(-1)
if int(mapelt.getAttribute('tilewidth')) != 32:
    print 'tile width must be 32, not', mapelt.getAttribute('tilewidth')
    sys.exit(-1)
...
if (int(tileset.getAttribute('tilewidth')) != 32):
    print 'tile width in tileset must be 32, not', \
            tileset.getAttribute('tilewidth')
    sys.exit(-1)
if (int(tileset.getAttribute('tileheight')) != 32):
    print 'tile height in tileset must be 32, not', \
...
if len(elt) != 4 * width * height:
    print 'map data is the wrong size: found', len(elt), \
        'but expected', (4 * width * height)
    sys.exit(-1)
